...
export class Persio implements INodeType {
	description: INodeTypeDescription = {
		...
		description: 'Consume Persio API',
		defaults: {
			name: 'Persio',
		},
		inputs: ['main'],
	...
	async execute(this: IExecuteFunctions): Promise<INodeExecutionData[][]> {
		...
		for (let i = 0; i < length; i++) {
			try {
				if (resource === 'group') {
					//https://persio.com/docs/connections/sources/catalog/libraries/server/http-api/#group
					if (operation === 'add') {
						...
						responseData = await persioApiRequest.call(this, 'POST', '/group', body);
					}
				}
				if (resource === 'identify') {
					//https://persio.com/docs/connections/sources/catalog/libraries/server/http-api/#identify
					if (operation === 'create') {
						const userId = this.getNodeParameter('userId', i) as string;
						const context = (this.getNodeParameter('context', i) as IDataObject)
							.contextUi as IDataObject;
						...
						responseData = await persioApiRequest.call(this, 'POST', '/identify', body);
					}
				}
				if (resource === 'track') {
					//https://persio.com/docs/connections/sources/catalog/libraries/server/http-api/#track
					if (operation === 'event') {
						const userId = this.getNodeParameter('userId', i) as string;
						...
						responseData = await persioApiRequest.call(this, 'POST', '/track', body);
					}
					//https://persio.com/docs/connections/sources/catalog/libraries/server/http-api/#page
					if (operation === 'page') {
						const userId = this.getNodeParameter('userId', i) as string;
						...
						responseData = await persioApiRequest.call(this, 'POST', '/page', body);
					}
				}
			} catch (error) {
